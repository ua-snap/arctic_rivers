#!/usr/bin/env python3
"""
Generate a SLURM .slurm job for calculating daily climatologies from combined Arctic Rivers NetCDF files.

Usage:
  python generate_climatology_job.py \
    /path/to/calculate_daily_climatology.py \
    /path/to/combined_wt.nc \
    /path/to/combined_q.nc \
    /path/to/wt_daily_clim_output.nc \
    /path/to/q_daily_clim_output.nc \
    /path/to/slurm/scripts \
    --conda-env "snap-geo" \
    --job-name "daily_climatology" \
    --partition "analysis" \
    --memory "750G" \
    --cpus 28 \
    --workers 6 \
...
Example submission:
    sbatch /path/to/slurm/scripts/daily_climatology.slurm
...
def main():
    parser = argparse.ArgumentParser(description="Generate daily climatology .slurm job file")
    parser.add_argument("climatology_script", help="Path to the calculate_daily_climatology.py script")
    ...
    parser.add_argument("wt_output_nc", help="Path to WT daily climatology output .nc file")
    parser.add_argument("q_output_nc", help="Path to Q daily climatology output .nc file")
    ...
    parser.add_argument("--job-name", default="daily_climatology")
    ...
    # Build climatology command
    climatology_cmd = (
        f"python {args.climatology_script} --wt {args.wt_input_nc} --q {args.q_input_nc} "
    ...
    if args.chunk_time > 0:
        climatology_cmd += f" --chunk-time {args.chunk_time}"
