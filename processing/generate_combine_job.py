...
Example submission:
    sbatch /path/to/slurm/scripts/combine_netcdf.slurm
...
def main():
    parser = argparse.ArgumentParser(description="Generate combine .slurm job file")
    parser.add_argument("combine_script", help="Path to the combine_arctic_rivers.py script")
    parser.add_argument("data_dir", help="Path to NetCDF files directory")
    parser.add_argument("wt_output_nc", help="Path to combined WT output .nc file")
    parser.add_argument("q_output_nc", help="Path to combined Q output .nc file")
    ...
    parser.add_argument("--job-name", default="combine_netcdf")
...
echo "Starting combining job at $(date)"
