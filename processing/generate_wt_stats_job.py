#!/usr/bin/env python3
"""
Generate a SLURM .slurm job for calculating stream temperature statistics from combined Arctic Rivers NetCDF files.

Usage:
  python generate_wt_stats_job.py \
    /path/to/calculate_wt_stats.py \
    /path/to/combined_wt.nc \
    /path/to/wt_stats_output.nc \
    /path/to/slurm/scripts \
    --conda-env "snap-geo" \
    --job-name "stream_temp_stats" \
    --partition "analysis" \
    --memory "750G" \
    --cpus 28 \
    --workers 6 \
...
Example submission:
    sbatch /path/to/slurm/scripts/stream_temp_stats.slurm
...
def main():
    parser = argparse.ArgumentParser(description="Generate stream temperature statistics .slurm job file")
    parser.add_argument("stats_script", help="Path to the calculate_wt_stats.py script")
    ...
    parser.add_argument("--job-name", default="stream_temp_stats")
    ...
    stats_cmd = (
        f"python {args.stats_script} --wt {args.wt_input_nc} "
        f"--wt-output {args.wt_output_nc} "
...
echo "Starting stream temperature statistics job at $(date)"
